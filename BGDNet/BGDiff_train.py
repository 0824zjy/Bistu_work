...
# Some environments fail at cuDNN handle initialization in the first Conv2d.
# Enable this by running with:
#   BGDNET_DISABLE_CUDNN=1 python BGDiff_train.py ...
import os
...
from BGDNet.models.BGDNet import BGDNet
from utils.dataloader_BGDiff import get_loader, test_dataset
from utils.utils import clip_gradient, adjust_lr, AvgMeter

warnings.filterwarnings("ignore")

# Work3 BEF-SBG freeze BatchNorm helper
def freeze_bn_if_needed(model):
    """
    When batchsize=1, some BatchNorm2d layers may receive [1,C,1,1],
    which is invalid in training mode. This helper keeps all BN layers
    in eval mode while the rest of the model still trains normally.
    ...
    if os.environ.get("BGDNET_FREEZE_BN", "0") != "1":
        return
...
def train(train_loader, model, optimizer, epoch, test_path, img_size=352, alpha=1.0, beta=0.4):
    ...
    # Work3 BEF-SBG apply freeze BN after model.train
    # model.train() will set BN back to train mode, so this must be called
    # immediately after model.train().
    freeze_bn_if_needed(model)
    global best
