...
class ControlNet(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Learnable boundary alpha.
        # Important:
        #   Old code initialized this as 0.0. Since boundary_hint_block ends with
        #   zero_module, boundary branch may receive extremely weak gradients at the
        #   beginning and can behave like a dead branch.
        #
        #   BOUNDARY_ALPHA_INIT allows shell scripts to control the initial strength.
        #   Recommended:
        #       Stage-1: BOUNDARY_ALPHA_INIT=0.5
        #       Stage-2: loaded from Stage-1 checkpoint
        boundary_alpha_init = _env_float("BOUNDARY_ALPHA_INIT", 0.5)
        self.boundary_alpha = nn.Parameter(
            torch.tensor(float(boundary_alpha_init), dtype=torch.float32)
        )
...
class ControlLDM(LatentDiffusion):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.boundary_prior_mode not in ["none", "hard", "soft", "external", "adaptive"]:
            raise ValueError(
                f"Unsupported BOUNDARY_PRIOR_MODE={self.boundary_prior_mode}. "
                "Valid values are: none, hard, soft, external, adaptive."
            )
    ...
    def _make_legacy_boundary_prior_when_external_missing(
        self,
        x: torch.Tensor,
        input_is_boundary: bool = False,
    ) -> torch.Tensor:
        """
        Fallback used only when BOUNDARY_PRIOR_MODE is external/adaptive
        but batch does not provide explicit boundary_prior.

        This keeps old SBG-Diff behavior:
        - if input is boundary-like, build soft prior around hard boundary;
        - if input is mask-like, extract boundary from mask then build soft prior.

        This prevents external/adaptive mode from accidentally treating mask itself
        as boundary prior when boundary_prior is absent.
        """
        if x is None:
            return None
        ...
        if input_is_boundary:
            if self.enable_soft_boundary_prior:
                return self._soft_boundary_prior_from_hard_boundary(x)
            return (x > 0.5).float().to(dtype=x.dtype)

        if self.enable_soft_boundary_prior:
            return self._soft_boundary_prior_from_mask(x)
    ...
    def _make_boundary_prior(
        ...
        """
        Convert input mask/boundary/external prior to selected boundary prior.

        BOUNDARY_PRIOR_MODE:
            none:
                return all-zero prior.
            ...
            external / adaptive:
                x is already an external adaptive soft boundary prior.
                Do NOT threshold.
                Do NOT extract boundary again.
                Do NOT soften again.
        """
        if x is None:
            return None
        ...
        if self.boundary_prior_mode in ["external", "adaptive"]:
    ...
    def get_input(self, batch, k, bs=None, *args, **kwargs):
        ...
        # Explicit boundary_prior:
        # In BEF-SBG, this is adaptive_boundary_prior from teacher feedback.
        # It must be used directly when mode is external/adaptive.
        if "boundary_prior" in batch:
            boundary_prior = batch["boundary_prior"]
        ...
        # No explicit adaptive prior, but old boundary exists.
        elif self.boundary_key is not None and self.boundary_key in batch:
            boundary_prior = batch[self.boundary_key]
            ...
            if self.boundary_prior_mode in ["external", "adaptive"]:
                boundary_prior = self._make_legacy_boundary_prior_when_external_missing(
                    boundary_prior,
                    input_is_boundary=True,
                )
            else:
        ...
        # No explicit prior and no boundary: old SBG-Diff fallback from mask.
        else:
            if self.boundary_prior_mode in ["external", "adaptive"]:
                boundary_prior = self._make_legacy_boundary_prior_when_external_missing(
                    control_mask,
