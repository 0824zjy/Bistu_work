...
class MyDataset(Dataset):
    """
    BEF-SBG inference dataset.

    target may be absent. If absent, use zero RGB placeholder so that
    ControlLDM.log_images reconstruction path will not crash.
    """

    def __init__(
        self,
        prompt_json="./data/prompt_test.json",
        size=384,
        fallback_radius=12,
        fallback_tau=4.0,
    ):
        ...
        print(f"[BEF Sample Dataset] json={self.prompt_json}, samples={len(self.data)}, size={self.size}")
    ...
    def __getitem__(self, idx):
        ...
        if target_filename is not None and os.path.exists(target_filename):
            target = np.array(Image.open(target_filename).convert("RGB")).astype(np.uint8)
        else:
            h, w = source_bin.shape
